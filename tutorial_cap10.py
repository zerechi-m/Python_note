...
# 10.12 ) 電池付きであること
 ...
 # pythonには「電池付き」の哲学がある。これがもっともよく表れているのが大きなパッケージにおける洗練された頑健な機能群である。

 # ・xmlrpc.client 及び xmlrpc.server モジュールを使用すればわずかな手間でリモートプロシンジャーコールが実装できる
 #   このようなXMLの処理や知識が直接は必要ない

 # ・emailパッケージはメールメッセージを処理するライブラリのパッケージでMIMEその他のRFC2822ベースのメッセージ処理できる。
 #   smtplibやpoplibとは違い、メッセージの送受信はしない。
 #   emailパッケージとは複雑なメール構造の構築やデコード、さらにはインターネットエンコーディングやヘッダプロトコルを実装するための完全なツールセットなのである。

 # ・jsonパッケージはこのポピュラーなデータ交換フォーマットのパースに安定したサポートを提供する、csvモジュールはデータベースやスプレッドシートでサポートされているcsv形式の
 #  ファイルの直接の読み書きをサポートする
 #  XMLの処理は xml.etree.ElementTree 及び xml.dom xml.sax の各パッケージでサポートされているこれらを使えばpythonアプリケーションと他のツールのデータ交換がやたら可能になる
